...
/** Update the independent, manual review state for a real-test mistake. */
export function markMistakeReview({ id, status = 'reviewed', nextPracticeDate = null, reviewedAt } = {}) {
  const allowed = new Set(['unreviewed', 'practice', 'reviewed']);
  const nextStatus = allowed.has(status) ? status : 'reviewed';
  let found = null;
  for (const entry of data.ielts) {
    for (const subject of ['listening', 'reading']) {
      ...
      const mistake = section.mistakes.find((item) => item && item.id === id);
      if (mistake) {
        found = mistake;
        break;
      }
  ...
  const previous = found.mistakeReview || {};
  found.mistakeReview = {
    status: nextStatus,
    lastReviewedAt: now,
    reviewCount: Math.max(0, Number(previous.reviewCount) || 0) + 1,
    nextPracticeDate: nextStatus === 'practice' ? (nextPracticeDate || addDaysIso(now, 1)) : null,
  };
  data.reviewLogs.unshift({
    ...
    framework: 'manual',
    itemId: id,
    rating: nextStatus === 'reviewed' ? 4 : 2,
    reviewedAt: now,
    nextDue: found.mistakeReview.nextPracticeDate || now,
    wasNew: false,
  });
...
function normalizeMistakeLocal(m) {
  ...
  if (typeof m === 'string') {
    ...
    return {
      ...
      review: null, mistakeReview: null, createdAt: '',
    };
  }
...
function normalizeMistakeReview(value) {
  ...
  const status = ['unreviewed', 'practice', 'reviewed'].includes(value.status)
    ? value.status
    : 'unreviewed';
  const lastReviewedAt = validIso(value.lastReviewedAt);
  const nextPracticeDate = validIso(value.nextPracticeDate);
  return {
    status,
    lastReviewedAt: lastReviewedAt || null,
    reviewCount: Math.max(0, Math.round(Number(value.reviewCount) || 0)),
    nextPracticeDate: status === 'practice' ? (nextPracticeDate || null) : null,
  };
}
