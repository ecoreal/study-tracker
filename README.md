...
- **智能学习教练**：根据逾期任务、近期专注、任务完成率和雅思短板推荐下一步与本轮时长
...
- **538 考点词**：项目内提供的 538 JSON 会在同步完成后首次自动导入，不重复写入
- **统计**：连续打卡、近 7 日专注柱状图、近 16 周热力图、基于近 14 日数据的智能复盘
- **爱听写同步**：浏览器书签一键同步阅读/听力错题本与阅读做题成绩（band 自动换算），本地保留同步历史
- **考试倒计时**：设置考试日期后，仪表盘显示剩余天数（考前 7 天高亮提醒）
- **Gist 同步**：多设备通过 PAT 读写同一私有 Gist
- **导入 / 导出 JSON** 备份

> 各版本详细改动见 [CHANGELOG.md](CHANGELOG.md)；修改同步 bridge 前请先读其中的「关键约定」。
