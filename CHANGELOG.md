...
## 2026-08-17（下午）—— 词汇系统智能化两轮

### 智能词汇学习流（`e26abac`）
对标不背单词：每日新词预算（默认 10，0=暂停；按 wasNew 日志扣减）、新词学习卡两步流（先学后测、压队尾）、四选一释义选择（同章节干扰项优先）、忘记重排队（每词≤2 次）、自动朗读（英音默认）。设置新增「词汇复习」区（模式/每日新词/朗读/口音）。

### 同替配对训练（`c1f34ca`）
538 的核心是替换网络，不是词义记忆：导入时保留结构化 `synonyms[]`/`headword`（种子 538-v2 幂等迁移存量）；新默认模式**同替配对**（英文↔英文四选一，双向出题，干扰项取其他词群成员并整群排除）；学习卡与答案揭示显示完整同替链 chips。真实数据验证 288 词可配对。

### 考前智能节奏（`c1f34ca` 同批）
考试日期已设且按当前节奏考前过不完第一轮时，复习总览出现建议横幅（剩余词数 ÷ 剩余天数）一键调整每日新词；计划行附带考试倒计时。

### 事故记录
一次 `git add -A` 误把目录里 80MB 的原书 PDF 提交并短暂部署（约 1 分钟），已 soft reset + force push 清除、`*.pdf` 进 gitignore、线上确认 404。**教训：提交用显式文件清单，勿 add -A。**
